...
MAX_THREAD_WORKERS = min(32, os.cpu_count() * 5)  # Higher thread count for I/O bound tasks
MODEL_NAME = "nvidia/nv-rerankqa-mistral-4b-v3"
API_KEY = "nvapi---"  # TODO: Implement API key rotation

# Resource management
torch.set_num_threads(4)  # Prevent excessive CPU usage per process
...
def distribute_questions(
    ...
    """Distribute questions for parallel processing using MPI."""
    total_questions = len(questions)

    # For single process or MPI master
    if size == 1 or rank == 0:
        logger.info(f"Processing {total_questions} questions with {size} MPI processes")

        # If using MPI, distribute work
        if size > 1:
            # Calculate work distribution
            chunk_size = total_questions // size
            remainder = total_questions % size

            start_idx = 0
            for i in range(1, size):
                end_idx = start_idx + chunk_size + (1 if i < remainder else 0)
                comm.send(questions[start_idx:end_idx], dest=i)
                start_idx = end_idx

            # Master process takes its chunk
            my_questions = questions[start_idx:]
        else:
            my_questions = questions
    else:
        # Worker nodes receive their chunk
        my_questions = comm.recv(source=0)

    # Process assigned questions with process pool
    results = []
    if my_questions:
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for result in tqdm(
                    executor.map(process_func, my_questions),
...
def run_parallel_pipeline(
    ...
    """Run the complete pipeline in parallel."""
    start_time = time.time()

    # Only rank 0 reads the questions file
    questions = []
    if rank == 0:
    ...
    results = distribute_questions(
        questions, use_topic_combiner, use_mxbai_reranker, use_query_refinement
    )

    # Only rank 0 handles output
...
if __name__ == "__main__":
    ...
    run_parallel_pipeline(
        ...
        output_path="../data/answer/fast_answers.jsonl"
    )