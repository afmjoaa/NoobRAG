...
def cluster_documents_with_berttopic(documents, min_topic_size=1, nr_topics="auto"):
    """
    ...
    for topic, indices in topic_groups.items():
        ...
        # If only one document in cluster, keep doc_id as string
        if len(indices) == 1:
            cluster['doc_id'] = doc_ids[0]
        else:
            cluster['doc_id'] = doc_ids
...
if __name__ == "__main__":
    ...
    doc_count = len(example_input)
    print(f"- Total documents: {doc_count}")
    print(f"- Number of clusters: {cluster_count}")

    # Count documents with arrays as doc_id
    combined_clusters = sum(1 for item in result if isinstance(item['doc_id'], list))
    print(f"- Number of clusters with combined documents: {combined_clusters}")

    # List clusters with their topics
    print("\nTOPIC DETAILS:")
    for i, cluster in enumerate(result):
        doc_id_display = f"[{len(cluster['doc_id'])} docs]" if isinstance(cluster['doc_id'], list) else "[1 doc]"
        print(f"Cluster {i + 1}: {doc_id_display}")


# Example input data with clear topics that should be combined
# example_input = [
